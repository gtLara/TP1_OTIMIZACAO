...
variables = ["num_iterations", "elapsed_time", "max_value_error", "solution_error"]
valb = 5
...
for var in variables: # instancia lista para tabelas
    tables[var] = []
    if "error" not in var: hybrid_tables[var] = []
...
for tol in duality_gap_grid:
    for step in step_grid:
        ...
        for dim in dimension_grid:
            ...
            optimum_x = sol_hybrid["solution"]["simplex"]

            sol_ip = interior_point(A, b, c, alpha0=step, tolerance=tol, optimum=optimum, optimum_x=optimum_x)

            # for n it only
            for var in variables:
                table_rows[var].append(sol_ip[var])
                if "error" not in var: hybrid_table_rows[var].append(sol_hybrid[var]["total"])

        for var in variables:
            tables[var].append(table_rows[var])
            if "error" not in var: hybrid_tables[var].append(hybrid_table_rows[var])
